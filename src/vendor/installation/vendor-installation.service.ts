...
export class VendorInstallationService {
  ...
  async updateStatus(
    ...
    // Validate status transitions - vendors can set IN_PROGRESS, COMPLETED, or REJECTED
    if (
    ...
    // Once rejected, cannot change to other statuses
    if (
    ...
    // Once completed, cannot be rejected
    if (
      installation.status === InstallationStatus.COMPLETED &&
      updateStatusDto.status === InstallationStatus.REJECTED
    ) {
    ...
    // If status is COMPLETED, set verifiedAt
    if (
      updateStatusDto.status === InstallationStatus.COMPLETED &&
