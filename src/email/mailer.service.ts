...
export class MailerService {
  ...
  constructor(private readonly configService: ConfigService<AllConfigType>) {
    ...
    this.transporter = nodemailer.createTransport({
      ...
      port,
      secure:
        this.configService.get<boolean>('mail.secure', { infer: true }) ??
        false,
      requireTLS:
        this.configService.get<boolean>('mail.requireTLS', { infer: true }) ??
        true,
      ignoreTLS:
        this.configService.get<boolean>('mail.ignoreTLS', { infer: true }) ??
        false,
      auth: { user, pass },
    });
  }
